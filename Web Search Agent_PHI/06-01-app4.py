...
model=Groq(id="llama-3.1-8b-instant", api_key=GROQ_API_KEY) #"llama3-groq-70b-8192-tool-use-preview")
...
def describe_data(filepath: str) -> str:
    ...
    # args: filepath (str): Path to CSV or Excel file
...
if uploaded_file is not None:
    try:
        ...
        if file_type in ["csv", "xlsx", "xls"]:
            ...
            response: RunResponse = agent.run(f"Describe the dataset in {temp_path}")
            # pprint_run_response(response, markdown=True)
            st.markdown("## Dataset Summary from Agent:")

...
# import requests
import logging
from getStockPriceYFinance import getStockPrice
# from getStockPriceAlphaVantage import getStockPrice
...
async def collect_user_data(state: FinanceState) -> FinanceState:
    ...
    response = await llm.ainvoke(prompt)  # llm.invoke(prompt)
...
async def get_stock_info(state: FinanceState) -> FinanceState:
    ...
    if not re.match(r'^[A-Z]{1,5}$', stock_symbol) or stock_symbol == 'UNKNOWN':
        ...
        try:
            # close_price, message = getStockPrice(stock_symbol, ALPHA_VANTAGE_API_KEY, logger)
            close_price, message = getStockPrice(stock_symbol)
...
graphBytes = finance_bot.get_graph().draw_mermaid_png()
with open("finadvisorGraph.png", "wb") as f:
    f.write(graphBytes)

# # Rendering the graph in Colab
# from IPython.display import Image, display
# display(Image(finance_bot.get_graph().draw_mermaid_png()))
